...
Status LoadUDG_AdjMatrix(MGraph *G,char *filename){
    ...
    for (int i = 0; i < G->vexnum;i++){//初始化邻接矩阵
        for (int j = 0; j < G->vexnum;j++){
            G->arcs[i][j].adj = 0;
            if(G->arcs[i][j].info!=NULL){//如果原来弧单元有其他信息，就释放掉
...
Status CreateUDN_AdjMatrix(MGraph *G){
    ...
    for (int k = 0; k < G->arcnum;k++){
        ...
        p2 = LocateVex_AdjMatrix(*G, v2);
        if(p1>=0&&p2>=0){
            G->arcs[p1][p2].adj =G->arcs[p2][p1].adj= adj;
            if(IncInfo){
                printf("Please input the extra information for arc[%c][%c]:\n", v1, v2);
                scanf("%d", G->arcs[p1][p2].info);
                G->arcs[p2][p1].info = G->arcs[p1][p2].info;
            }
        }else{
...
//保存一个用邻接矩阵表示的无向网
Status SaveUDN_AdjMatrix(MGraph G,char *filename){
    FILE *fp = fopen(filename, "w");
    if(!fp){
    ...
    for(int i=0;i<G.vexnum;i++){
        for(int j=i;j<G.vexnum;j++){
            if(G.arcs[i][j].adj!=INFINITY){
                fprintf(fp, "<%c,%c> weight:%d",G.vexs[i],G.vexs[j],G.arcs[i][j].adj);
                if(IncInfo){
                    fprintf(fp, " Info:%d\n",*G.arcs[i][j].info);
                }else{
                    fprintf(fp, " Info:NULL\n");
...
Status DFSTranverAMLGraph(AMLGraph G,Status (*visit)(VertexType vertex)){
    ...
    int v1;
    EdgeBox *p;
    for(int i=0;i<G.vexnum;i++){//为防止G是非连通图，应当将每个顶点当做根，访问一遍
        if(!visitmark[i]){//如果没被访问过，就开始遍历
            v1 = i;//v1指针赋值为作为深度遍历时这一颗最小生成树的根(i)
