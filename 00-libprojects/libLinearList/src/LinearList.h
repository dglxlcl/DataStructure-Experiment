...
typedef struct{//顺序表定义，结构成员为：ElemType *elem，int length，int listsize
...
extern Status InitList_Sq(Sqlist *L);//构造一个空的顺序表，长度初始化为0，分配LISE_INIT_SIZE长度的空间给*elem，返回OK（1），若分配失败，返回ERROR（0）
...
extern Status GetElem_Sq(Sqlist L,int i,ElemType *e);
...
extern Status MakeDuLNode(DuLink *p,ElemType e);
...
extern Status DelFirst_L(LinkList_L *L,Link *q);
